...
export const getUsersByRol = async (role_name, status) => {
    ...
    const [result] = pool.query(
        'SELECT subscriber.id, subscriber.name, subscriber.lastname, subscriber.dni, institution, nation, category FROM subscriber JOIN users ON users.id = subscriber.fk_user JOIN roles ON roles.id = users.rol WHERE users.status = 1 AND rol_name = ?',
        [role_name]
    );
    return result;
};
